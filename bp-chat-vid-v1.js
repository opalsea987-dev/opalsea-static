(function () {
  const WEBHOOK_URL = "https://n8n.opalsea.site/webhook/02fea21d-7d90-46fb-b12d-1cd7dd79575a";
  ...
  style.textContent = `
    @keyframes opalsea-float {
      ...
      25%  { transform: translate(1px,-6px) scale(1.02) rotate(1deg); }
      50%  { transform: translate(-1px,-3px) scale(0.99) rotate(-1deg); }
      75%  { transform: translate(2px,-7px) scale(1.01) rotate(0.5deg); }
  ...
  btn.innerHTML = `
    <div id="opalsea-avatar-wrap">
      <video id="opalsea-avatar-video" autoplay loop muted playsinline poster="${AVATAR_URL}">
  ...
  win.innerHTML = `
    <div id="opalsea-chat-header">
      <img src="${AVATAR_URL}" alt="Lia" />
      <span>Lia | Belrose Place</span>
